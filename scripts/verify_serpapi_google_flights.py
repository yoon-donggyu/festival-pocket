...
def choose_candidates(top100,previous):
 now=datetime.now(timezone.utc); by_dest={}
 ...
 for rank,d in enumerate(top100,1):
  ...
  score=0.; reasons=[]
  if not prev:score+=40;reasons.append('신규/미검증')
  ...
  score+=max(0,21-rank)
  if rank<=20:reasons.append(f'TOP{rank}')
...
def main():
 ...
 fares=load_json(FARES_PATH,{}); top=fares.get('top100')or fares.get('top50')or[]; old=load_json(OUT_PATH,{'results':[]}); previous={r.get('candidate_key'):r for r in old.get('results',[]) if r.get('candidate_key')}; selected=choose_candidates(top,previous); results=dict(previous); run=[]; now=datetime.now(timezone.utc).isoformat()
 for priority,rank,d,reasons in selected:
  ...
  try:
   ...
   for g in groups:
    for leg in g.get('flights')or[]:
     ...
     if target and fn==target:exact.append({'price':g.get('price'),'airline':leg.get('airline'),'flight_number':leg.get('flight_number'),'departure':(leg.get('departure_airport')or{}).get('time'),'arrival':(leg.get('arrival_airport')or{}).get('time')})
  ...
  results[k]=row;run.append(row);print(f"SERP {d.get('origin')}->{d.get('destination')} url={'yes' if row.get('google_flights_url') else 'no'}")
 kept=sorted(results.values(),key=lambda r:r.get('checked_at_utc')or'',reverse=True)[:500]; out={'generated_at_utc':now,'quota_policy':{'runs_per_day':2,'queries_per_run':MAX_QUERIES,'approx_monthly_30d':MAX_QUERIES*60,'recheck_hours':RECHECK_HOURS},'queries_this_run':len(run),'run_results':run,'results':kept};OUT_PATH.parent.mkdir(parents=True,exist_ok=True);OUT_PATH.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8');print(f'SerpApi cross-check complete: {len(run)} queries')
