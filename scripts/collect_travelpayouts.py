...
# 국제선은 인천/김포, 제주 국내선은 김포/청주/부산/대구 출발까지 함께 탐색한다.
ORIGINS = ["ICN", "GMP"]
JEJU_ORIGINS = ["GMP", "CJJ", "PUS", "TAE"]
DESTINATIONS = {
...
def main():
        ...
        for month in MONTHS:
            ...
            try:
                ...
                for row in rows:
                    ...
                    if not n: continue
                    # 제주도는 주말여행까지 잡기 위해 1~7박, 국제선은 2~14박.
                    if n["nights"] is not None:
                        if destination=="CJU" and not (1 <= n["nights"] <= 7): continue
                        if destination!="CJU" and not (2 <= n["nights"] <= 14): continue
                    deals.append(n); accepted += 1
                stats.append({"origin":origin,"destination":destination,"month":month,"returned":len(rows),"accepted":accepted})
                time.sleep(0.12)
    ...
    best={}
    for d in deals:
        key=(d["origin"],d["destination"]) if d["destination"]=="CJU" else d["destination"]
        if key not in best: best[key]=d
    now=datetime.now(timezone.utc).isoformat()
    output={"generated_at_utc":now,"note":"Travelpayouts Data API 가격은 Aviasales 사용자 검색 이력 기반 캐시 가격이며 실시간 전체 재고가 아닙니다. 실제 결제 전 재확인이 필요합니다.","origins":ORIGINS,"jeju_origins":JEJU_ORIGINS,"months":MONTHS,"deal_count":len(deals),"top100":top100,"top50":top100[:50],"best_by_destination":list(best.values()),"all_2to14_night_deals":deals,"all_2to7_night_deals":deals}
